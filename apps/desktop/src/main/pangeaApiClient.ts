...
/** The account is fine, the subscription ran out. Deliberately not an AuthError:
 *  that logs the user out and burns a device slot on every lapsed crypto account. */
...
/** Deduplicated, non-empty entries, order preserved — node endpoint addresses
 *  from the hub can repeat when several transports share the node's IP. */
...
// Every sealed envelope goes out as JSON; shared so the fetch calls below don't repeat it.
const SEALED_ENVELOPE_HEADERS = { "Content-Type": "application/json" } as const;
...
/** POST the envelope to an edge relay, which forwards it to the hub. Unlike
 *  fetchDohResolved this validates the cert normally: the relay's name is real. */
...
/** Splits `block` into sibling sub-blocks covering it minus host `ip`. Generalized
 *  from 0.0.0.0/0 so allowedIPsExcludingAll can recurse into an already-split range. */
...
/** AllowedIPs CIDRs covering 0.0.0.0/0 minus every IP in `excludeIPs` — each
 *  transport's own outbound connection must stay off the tunnel or routing loops. */
...
export class PangeaApiClient {
  ...
  // persisted so a client that cannot reach the hub still has somewhere to connect.
  ...
  // Last hub IP that worked, restored at startup — the only path in while a
  // Lockdown lock permits it but blocks DNS/DoH. Survives resetHubResolution().
  ...
  // Consecutive hubFetch failures on the currently-resolved path — once this hits
  // HUB_FAILURE_LIMIT the path is presumed stale and hubReady is cleared to re-run.
   ...
   *  at-least-one-enabled invariant is enforced by the caller (shared/hubMethods.ts). */
   ...
   *  rejected — invalid input leaves the current value alone, not the default. */
   ...
   *  path; uses /api/client/regions since the hub rejects unauthenticated /health with 403. */
  ...
  /** Ensures a working connection strategy to the hub API, trying enabled methods in
   *  order: cached/DoH IP, shadowsocks, fronted relay, then cleartext domain last. */
  ...
  /** Pulls the signed public bootstrap file and folds it into the caches the ladder
   *  reads; returns whether anything changed. Never throws — an unreachable drop is a no-op. */
  ...
  /** Unified fetch that encrypts all requests through the secure channel: net.fetch
   *  for direct domain (trusts system cert store), fetchDohResolved for a DoH IP. */
  ...
  private async resolveRawResponse(
    ...
    try {
      ...
      if (!this.hubMethods.directIp) throw err;
      const resolvedIp = await resolveViaDoH(HUB_HOSTNAME);
      if (!resolvedIp) throw err;
      this.dohResolvedIp = resolvedIp;
      this.rememberHubIp(resolvedIp);
      this.setActiveHubMethod("directIp", resolvedIp);
  ...
  /** The node list, falling back to the cached one when the hub is unreachable.
   *  An AuthError or expired subscription still propagates — that's the hub answering "no". */
  async getServers(): Promise<ServerInfo[]> {
    if (!this.licenseKey) throw new Error("Not authenticated");
    // Send identityPubkey so the hub returns this device's own cloakUid and re-pushes
    // it to every region. Older hubs ignore the param and fall back to LIMIT-1.
  ...
  /** Records the node list the hub just gave us. An empty list is ignored — more
   *  likely a truncated response or a mid-deploy hub than an instruction to forget servers. */
  ...
  async provision(
    ...
    // Transports terminate on the entry node; `server` stays the entry below,
    // since every credential block describes where a transport dials.
    ...
    // Transport endpoints must be excluded from AllowedIPs and permitted through the kill
    // switch — all from the hub (IPv4 literals only; the split does integer arithmetic).
    ...
    return {
      ...
      // Reality and Hysteria2 dial the node's IP rather than its domain, so no lookup
      // stands between them and the node — the difference behind a Lockdown lock.
      ...(server.reality ? {
        reality: {
          localPort: 0,
      ...
      wireguard: {
        ...
        // The hub's IP, so the daemon keeps it reachable outside the tunnel and through
        // the kill switch; falls back to the last known good IP if reached by domain.
        bypassHosts: (() => {
  ...
  clearCache(): void {
    ...
    this.cachedServers = [];
    // The node list is account-scoped, so it goes when the account does. The hub IP,
    // relays and control-plane credentials survive: they let a signed-out client sign in again.
    this.onServers?.([]);
   ...
   *   (e.g. Stop mid-connect), composed with the timeout controller so either can abort. */
