...
export const ru = {
  ...
  "settings.censorship.description": "Как приложение связывается с серверами вашей учётной записи. Каждый включённый способ пробуется по очереди; хотя бы один должен остаться включённым.",
  ...
  "settings.network.dns.provider.hint": "Выберите DNS-сервис или укажите свой. Фильтрация выполняется выбранным сервисом. Вступает в силу при следующем подключении.",
  ...
  "settings.network.dns.hint": "IPv4-адреса DNS через запятую, например 1.1.1.1, 1.0.0.1. Оставьте пустым, чтобы использовать DNS VPN по умолчанию. Вступает в силу при следующем подключении.",
  ...
  "settings.network.mtu.hint": "Размер пакетов, отправляемых через туннель. Уменьшите его, если страницы зависают или большие загрузки останавливаются в сети, которая в остальном подключается нормально. По умолчанию 1380. Вступает в силу при следующем подключении.",
