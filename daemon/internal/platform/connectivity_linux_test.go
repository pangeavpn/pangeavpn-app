...
func TestHasPhysicalDefaultRoute(t *testing.T) {
	...
	def := func(oif int) linuxRoute {
		return linuxRoute{dstLen: 0, table: unix.RT_TABLE_MAIN, routeTyp: unix.RTN_UNICAST, oif: oif}
	}
	...
	cases := []struct {
		...
		{"tunnel half-default is not a default", []linuxRoute{{dstLen: 1, table: unix.RT_TABLE_MAIN, routeTyp: unix.RTN_UNICAST, oif: 2}}, false},
		{"non-main table default ignored", []linuxRoute{{dstLen: 0, table: 51820, routeTyp: unix.RTN_UNICAST, oif: 2}}, false},
		{"blackhole default ignored", []linuxRoute{{dstLen: 0, table: unix.RT_TABLE_MAIN, routeTyp: unix.RTN_BLACKHOLE, oif: 2}}, false},
