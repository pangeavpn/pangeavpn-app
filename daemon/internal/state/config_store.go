...
// configSchemaVersion guards fields added after this build; a file with a
// newer version loads as-is but is never rewritten, so downgrades can't lose data.
const configSchemaVersion = 1

// configFile is the on-disk envelope around Config, kept here rather than in
// types.go so persistence can carry a schema version independently.
...
// decodeConfig parses raw, quarantining invalid profiles instead of failing
// the whole load, and reports whether the file should be rewritten to match.
...
// recoverFromBackup runs when the primary config is empty or unparsable; it
// never falls back to DefaultConfig, only to config.json.bak or failure.
...
// cleanupStaleTemp removes ".tmp-*" files left by a persist that crashed
// before its rename; safe since each persist uses a fresh unique name.
...
// persistLocked writes cfg via a uniquely-named temp file, backs up the
// previous config, then atomically renames the temp file over the target.
...
// fileLock is an exclusive-create lock guarding config.json's read-modify-write
// against a second daemon, installer, or CLI touching the same path.
...
// can reject port collisions instead of failing later at transport startup.
...
// quarantineInvalidProfiles is ValidateConfig's non-fatal counterpart for
// loading from disk: an invalid profile is dropped and logged, not fatal.
...
// profilePortClaims lists ports the profile's configured transports bind to.
// Cloak is only checked when RemoteHost is set; other transports may be nil.
...
// cloneNaiveProfile returns an independent copy behind a fresh pointer, so
// callers can't mutate the store's internal state without its lock.
...
// cloneSnowflakeProfile mirrors cloneNaiveProfile but also deep-copies
// FrontDomains and ICEServers so no backing array is shared with the store.
